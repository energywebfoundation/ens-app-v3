...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (owner !== deployer) {
    ...
    console.log(`Owner of ETHRegistrarController set to ${owner} (tx: ${tx.hash})...`)
    await tx.wait()
  }
  ...
  console.log(`ETHRegistrarController added to controllers of NameWrapper (tx: ${tx1.hash})...`)
  await tx1.wait()

  const tx2 = await reverseRegistrar.setController(controller.address, true)
  console.log(
    `ETHRegistrarController added to controllers of ReverseRegistrar (tx: ${tx2.hash})...`,
  )
  await tx2.wait()
  ...
  // const resolverContract = await ethers.getContractAt('PublicResolver', resolver)
